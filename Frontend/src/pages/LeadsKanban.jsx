        ...
        <div className="d-flex gap-3 align-items-center">
          ...
          {isTrashMode && isAdmin && filters.userId && leads.length > 0 && (
            <Button
              variant="success"
              onClick={() =>
                handleBulkAction(filters.userId, "RESTORE_ALL", userRole, {
                  userId: filters.userId,
                  softDeleted: true,
                })
              }
          ...
          {/* Botões que só aparecem Na Lixeira (Admin) */}
          {isTrashMode && isAdmin && filters.userId && leads.length > 0 && (
            <Button variant="danger" onClick={openEmptyTrashConfirm}>
            ...
            >
              ...
              <div className="d-flex flex-column gap-2">
                {leadsNaColuna.map((lead) => (
                  <Card
                    key={lead.id}
                    className="shadow-sm border-0 kanban-card"
                  >
                    <Card.Body className="p-3">
                      ...
                      <div className="d-flex justify-content-between align-items-end border-top pt-2 mt-2">
                        <div>
                          {/* ADICIONADO: Nome Completo e Ícone para facilitar a gestão do Admin */}
                          <div
                              ...
                              <>
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (
                                      window.confirm(
                                        "Esta ação é irreversível. Deseja mesmo eliminar?",
                                      )
                                    ) {
      ...
      >
        ...
        {modalConfig.type === "DELETE_CONFIRM" && (
          <div className="p-3 text-center">
            ...
            ></i>
            <p className="mt-3">
              Tem a certeza que deseja enviar a lead{" "}
              <strong>{modalConfig.data?.title}</strong> para a lixeira?
            </p>
        ...
        {modalConfig.type === "BULK_DELETE" && (
          <div className="p-3 text-center">
            ...
            <p>
              Tem a certeza que deseja mover <strong>TODAS</strong> as leads
              deste utilizador para a lixeira?
            </p>
            <div className="d-flex gap-2 justify-content-center mt-4">
              <Button variant="secondary" onClick={closeModal}>
                Cancelar
              ...
              <Button
                variant="danger"
                onClick={async () => {
                  const success = await handleBulkAction(
                    filters.userId,
                    "SOFT_DELETE_ALL",
                    userRole,
                    { userId: filters.userId },
